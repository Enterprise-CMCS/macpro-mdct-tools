...
class TestScanTable:
    ...
    @patch("backend.routes.dynamodb.get_client")
    def test_filtered_scan_skips_non_matching_prefix(self, mock_get_client):
        """FilterExpression Limit is items read, not matches; keep scanning until a page fills."""
        ...
        with mock_aws():
            ...
            ddb.create_table(
                ...
                KeySchema=[{"AttributeName": "answer_entry", "KeyType": "HASH"}],
                AttributeDefinitions=[{"AttributeName": "answer_entry", "AttributeType": "S"}],
                BillingMode="PAY_PER_REQUEST",
            )
            for i in range(150):
                ddb.put_item(
                    TableName="form-answers",
                    Item={"answer_entry": {"S": f"AA-{i:04d}"}},
                )
            for i in range(12):
                ddb.put_item(
                    TableName="form-answers",
                    Item={"answer_entry": {"S": f"CO-{i:04d}"}},
                )
            mock_get_client.return_value = ddb

            resp = client.get(
                "/api/dynamodb/tables/form-answers/items",
                params={
                    "limit": 10,
                    "filter_attribute": "answer_entry",
        ...
        assert all(item["answer_entry"]["S"].startswith("CO-") for item in data["items"])
